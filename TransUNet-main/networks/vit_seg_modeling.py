...
def np2th(weights, conv=False):
    """可能将 HWIO 格式转换为 OIHW 格式。"""
    if conv:
        # 若为卷积权重，将其从 HWIO 转换为 OIHW
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        """
        将给定的权重加载到模型中。此函数主要用于将预训练的权重加载到模型的各个组件中，
        包括嵌入层、编码器、位置嵌入等。如果位置嵌入的大小不匹配，它将调整位置嵌入的大小以适应模型。
        
        参数:
        weights (dict): 包含预训练权重的字典。
        """
        with torch.no_grad():
